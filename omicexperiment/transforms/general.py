...
class Cluster(Transform):
    ...
    def __eapply__(self, experiment):
        ...
        if not (self.tax_assignment_file is None):
            ...
            new_unique_clusters = self.clusters_df['cluster'].drop_duplicates()
...
class ClusterIntoOTUs(Transform):
    ...
    def __process_assignment_file(self):
        ...
        obs_to_otu_dict = OrderedDict(observation=observation_list, otu=otu_list)

        del read_list
        del otu_list

        df = pd.DataFrame(obs_to_otu_dict, index=observation_list)
    ...
    def __eapply__(self, experiment):
        obs_otu_df = self.__process_assignment_file()
        new_data_df = experiment.data_df.join(obs_otu_df).groupby("otu").sum().reset_index().set_index("otu")
        ...
        if self.tax_assignment_file is not None:
            ...
            new_taxonomy_df.rename(columns={'observation':'otu_old'}, inplace=True)
